...
if ollama list 2>/dev/null | grep -q "$MODEL"; then
  ...
  ollama pull "$MODEL" 2>&1 || {
    ...
    ollama pull "$BASE_MODEL" 2>&1 || {
      ...
      echo "   - gemma3:1b  (1B params, ~800MB download)"
      echo "   - gemma3:4b  (4B params, ~2.6GB download)"
      echo "   - gemma2:2b  (2B params, ~1.5GB download)"
      echo "   - gemma2:9b  (9B params, ~5.3GB download)"
      exit 1
    }
  }
